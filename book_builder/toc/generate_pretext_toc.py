...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Export a PreTeXt table of contents to CSV by following xi:include files."
    )
    parser.add_argument(
        "root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Base directory used when writing source_path values",
    )
    parser.add_argument(
        "--resource-name",
        type=str,
        default=None,
        help=(
            "Short identifier prepended to every generated node ID "
            "(defaults to the uppercased root file stem, e.g. 'ORCCA')"
        ),
    )
    parser.add_argument(
        "--mapping-output",
